...
- [Development guidelines](#development-guidelines)
    - [General PWC Core](#general-pawcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
PWC Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
PWC Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/pawcoin-project/pawcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_pawcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_pawcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/pawcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_pawcoin.coverage/index.html`.
```
...
However, in open source software such as PWC Core, where everyone uses
...
pay attention to for reviewers of PWC Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: PWC Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against PWC Core so that they can be integrated
...
file descriptors it will cause problems with PWC's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:PWC-Project/PWC.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `pawcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `pawcoin-cli` and the GUI debug console use this table to determine how to
