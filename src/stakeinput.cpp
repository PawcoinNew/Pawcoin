...
// The zPWC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPWC is a hash of the serial
...
{
    //Create an output returning the zPWC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPIVOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPWC output", __func__);
...
{
    //The unique identifier for a PWC stake is the outpoint
