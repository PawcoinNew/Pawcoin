                                                        ...
                                                        model(0)
{
    ...
    //selection of first 625 inputs on button pressed
    connect(ui->select_625, SIGNAL(clicked()), this, SLOT(select_625()));
    ...
    ui->treeWidget->setColumnHidden(COLUMN_AMOUNT, true);         // store amount int64 in this column, but dont show it
    ui->treeWidget->setColumnHidden(COLUMN_PRIORITY, true); // store priority int64 in this column, but dont show it
    ui->treeWidget->setColumnHidden(COLUMN_DATE, true);     // store date int64 in this column, but dont show it
    ...
    ui->select_625->setVisible(false);
...
void CoinControlDialog::select_625() //select the first 625 inputs 
{
        ...
        if (ui->treeWidget->topLevelItem(i)->checkState(COLUMN_CHECKBOX) != state)
            ...
            ui->treeWidget->setEnabled(true);
                if (state == Qt::Unchecked)
                    ...
                    CoinControlDialog::updateLabels(model, this);
                    updateDialogLabels();
                }else{ //if we have less then 625 inputs give the user dialogue to inform them of this issue  
                    QMessageBox msgBox;
                    msgBox.setObjectName("lockMessageBox");
                    msgBox.setStyleSheet(GUIUtil::loadStyleSheet());
                    msgBox.setText(tr("Please have at least 625 inputs to use this function."));
                    msgBox.exec();
    }
}
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::PWC;
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show PWC address. In tree mode, address is not shown again for direct wallet address outputs
