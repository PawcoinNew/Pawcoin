    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zPWC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPWC seed used to mint this zPWC is not the same as currently hold in the wallet";
            else
