...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "pawcoin" is a composite category enabling all PWC-related debug output
            if (ptrCategory->count(std::string("pawcoin"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Pawcoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Pawcoin
// Mac: ~/Library/Application Support/Pawcoin
// Unix: ~/.pawcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pawcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "pawcoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override pawcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "pawcoind.pid"));
